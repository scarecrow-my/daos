...
dtx_19(void **state)
{
	...
	for (i = 0; i < DTX_TEST_SUB_REQS; i++) {
		...
		nr[i] = 1 << (i % 24);
		if (max_size < i_size * nr[i])
			max_size = i_size * nr[i];

		oid[i] = dts_oid_gen(oclass, 0, arg->myrank);
		ioreq_init(&req[i], arg->coh, oid[i], i_type, arg);

		D_ALLOC(write_bufs[i], i_size * nr[i]);
		assert_non_null(write_bufs[i]);

		dts_buf_render(write_bufs[i], i_size * nr[i]);
		insert_single_with_rxnr(dkey, akey, i, write_bufs[i],
					i_size, nr[i], th, &req[i]);
	}
	...
	for (i = 0; i < DTX_TEST_SUB_REQS; i++) {
		lookup_single_with_rxnr(dkey, akey, i, fetch_buf, i_size,
					i_size * nr[i], DAOS_TX_NONE, &req[i]);
		assert_memory_equal(write_bufs[i], fetch_buf, i_size * nr[i]);
	}
